...
export const PollRow = ({ props }) => {
    const { options, addOption, removeLastOption, updateOptionText } = useStore()
    const [commentsVisible, setCommentsVisible] = useState(false)
    const [voted, setVoted] = useState(false)
    const ref = useRef(null)

    const hasAnyVotes = true; //shown after poll is posted to DB
    const hasComments = true; //shown after poll is posted to DB

    const handleWsClick = useCallback((e) => {
        e.preventDefault()
        let targeRowId = e.target?.id;
        //update ui
        setVoted(!voted)

        //update ws state
        const socketReady = props.socket?.readyState === WebSocket.OPEN ? true : false;
        debugger
        console.log(`WS connection ready: ${socketReady}`);
        if (socketReady) {
            const currentCount = voted ? ++props.row.upvotes : --props.row.upvotes;
            debugger;
            props.socket.send(JSON.stringify({
                rowId: 123,
                Comment: {message:'tralalala',id:1},
                vote: { count: currentCount }//read updated count from UI State
            }))
        }
        //TODO: update DB on socket close (check if no clients are connected) than update DB
    ...
    function handleKeyPress(e) {
        switch (e.keyCode) {
            case 13://enter
                e.preventDefault();
                let idx = props?.index;
                //only add option if this is last row
            ...
            case 46://delete
                e.preventDefault();
                removeLastOption()
                //Focus on previous sibiling
                const previousOptionId = options[options.length - 1]?.id;
                if (previousOptionId !== null && previousOptionId !== undefined)
                    document.getElementById(`option-input-${previousOptionId}`).focus()
                else
    ...
    function handleOptionUpdate(params) {
        let option = options.find(o => o.id === props?.index)
        option.text = params.target.value;
        updateOptionText(option)
    }
    useEffect(() => {
        //fucus on input field when new option is added
        ref.current?.focus()
    }, [])

    return (
        <>
            <li className="flex items-center text-gray-600 dark:text-gray-200 justify-between border-b-2 border-gray-100 dark:border-gray-800">
                <div className="flex items-center justify-start text-sm">
                    <span className="mx-4">
                        {`${props.index + 1}`}
                    </span>
                    <span className={voted ? "font-bold text-gray-500 underline decoration-4 decoration-sky-500" : ""}>
                        {props.editMode ? <input id={`option-input-${props?.index}`} ref={ref} className="appearance-none bg-transparent border-b border-teal-400 w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none" placeholder='Poll option' type="text" aria-label='Poll option'
                            onKeyUp={(e) => handleKeyPress(e)} onChange={(e) => handleOptionUpdate(e)} value={props?.row?.text} />
                            : props?.row?.text}
                    </span>
                    {hasComments && !props.editMode && <>
                        <a className="hover:cursor-pointer" >
                            <span className={`p-2 lg:ml-6 ml-2 flex items-center ${commentsVisible ? 'text-teal-500' : 'text-gray-400'} dark:text-gray-300`} onClick={() => handleCommentsClick()}>
                                3
                        ...
                        {/* THREAD COUNT IS DISABLED FOR NOW ON ALL DEVICES */}
                ...
                <div className="flex items-center justify-end">
                    {hasAnyVotes && !props.editMode &&
                        <>{props.row.upvotes}
                            <svg width="20" fill="currentColor" height="20" className="h-3 text-teal-500" viewBox="0 0 1792 1792" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <a className="p-3 hover:cursor-pointer" onClick={(e) => handleWsClick(e)}>
                        {voted ?
                            <svg width="30" height="30" fill="currentColor" viewBox="0 0 1024 1024" className="text-teal-500">
                            ...
                            :
                            <svg width="30" height="30" fill="currentColor" className="text-gray-400 dark:text-gray-300" viewBox="0 0 1024 1024">
            ...
            {!props.editMode && commentsVisible &&
                <CommentSection />
            }
        </>
    )
